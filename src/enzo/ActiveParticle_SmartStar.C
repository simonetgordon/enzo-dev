...
#define MASSTHRESHOLD      35 //Msolar in grid. SG. changed to 10 to prevent runaway SF in EvaluateFormation.
    ...
    LevelHierarchyEntry *LevelArray[], int ThisLevel, int SmartStarID)
{
    ...
    for (i = 0; i < nParticles; i++) {
      ...
      if (MyProcessorNumber == FeedbackZone->ReturnProcessorNumber()) {
        ...
        if (BondiHoyleRadius*LengthUnits/pc_cm < SmartStarMinimumRefinementRadius){
          fprintf(stderr, "%s: BondiHoyle radius too small at %e pc (%f cells). Setting to "
                          "SmartStarMinimumRefinementRadius = %e pc (%e code).\n",
                  __FUNCTION__,BondiHoyleRadius*LengthUnits/pc_cm, BondiHoyleRadius/dx, SmartStarMinimumRefinementRadius,
                  SmartStarMinimumRefinementRadius*pc_cm/LengthUnits);
        ...
        fprintf(stderr, "%s: AccretionRadius = %e pc (%f cells), bondi_cells = %"ISYM", SmartStarBondiRadiusRefinementFactor = %"ISYM". \n", __FUNCTION__,
                AccretionRadius*LengthUnits/pc_cm, AccretionRadius/dx, bondi_cells, SmartStarBondiRadiusRefinementFactor);
      ...
      if (MyProcessorNumber == APGrid->ReturnProcessorNumber()) {
        ...
        if( ((ctime - SS->AccretionRateTime[SS->TimeIndex])*TimeUnits/yr_s > (float)TIMEGAP)
          ...
          fprintf(stderr, "accrate = %1.2e Msolar/yr\t deltatime = %3.3f yrs\t TimeIndex = %d\t "
                          "Particle Mass = %1.2e Msolar\t Age = %e yrs\t Lifetime = %1.2f Myr\t Class = %d\n",
