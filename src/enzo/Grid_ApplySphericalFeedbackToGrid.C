      ...
      for (int i = GridStartIndex[0]; i <= GridEndIndex[0]; i++, index++) {
        ...
        if (radius2 < outerRadius2) {
          ...
          if (GENum >= 0 && DualEnergyFormalism) {
            ...
            if(EjectaDensity > 0.0) { /* SuperNovae - no ramping */
              newGE = (OldDensity * this->BaryonField[GENum][index] +
               ...
               fprintf(stderr,"%s: oldGE = %"GSYM"\t newGE = %"GSYM"\t maxGE = %"GSYM" code units \n",
                       __FUNCTION__, oldGE, newGE, maxGE);
...
//              fprintf(stderr,"%s: dGE = %"GSYM" code units, \t GE_max = %"GSYM" code units, \t oldGE = %"GSYM" code \
//              units,\t SS->EnergySaved = %e code units \n",
